...
(() => {
  ...
  const tourData = {
    case: {
      ...
      items: [
        "Cadastro central do cliente",
        "Casos ligados ao mesmo histórico",
        "Documentos disponíveis no contexto"
      ],
      ui: [
        ["Maria S. Oliveira", "Benefício por incapacidade", "Cadastro central"],
        ["Documentos do caso", "Procuração · RG/CPF · laudo", "Tudo vinculado"],
        ["Próximo passo", "Conferir pendência documental", "Em contexto"]
      ]
    },
    judicial: {
      ...
      items: [
        "Origem administrativa preservada",
        "Pendências antes do protocolo",
        "Status de preparação visível"
      ],
      ui: [
        ["Origem", "Requerimento administrativo", "Preservado"],
        ["Pendências", "Documentos para conferência", "2 itens"],
        ["Judicial", "Preparação concluída", "Pronto para protocolo"]
      ]
    },
    word: {
      ...
      items: [
        "Arquivo DOCX do escritório",
        "Campos vinculados ao cliente",
        "Conteúdo e identidade preservados"
      ],
      ui: [
        ["modelo-procuracao.docx", "Modelo original do escritório", "DOCX"],
        ["Campos reconhecidos", "Nome · CPF · endereço", "Mapeados"],
        ["Saída", "Documento preenchido", "Pronto para revisar"]
      ]
    },
    finance: {
      ...
      ui: [
        ["Contrato previdenciário", "Honorários vinculados ao cliente", "Ativo"],
        ["Recebimento parcial", "R$ 3.500,00", "Registrado"],
        ["Visão gerencial", "Receitas · despesas · saldo", "Acesso controlado"]
      ]
    }
  };
  function renderTour(key = "case") {
    ...
    stage.innerHTML =
      '<div class="tour-card"><div class="tour-copy"><small>' +
      data.kicker +
      "</small><h3>" +
      data.title +
      "</h3><p>" +
      data.text +
      "</p><ul>" +
      data.items.map((x) => "<li>" + x + "</li>").join("") +
      '</ul></div><div class="tour-ui"><div class="mini-top"><span>FLUXO JURÍDICO</span><b>Fluxo conectado</b></div>' +
      data.ui
        .map(
          (x, i) =>
  ...
  function applyPlanUI(config) {
    ...
    $$("[data-plan-price]").forEach((el) => {
      const p = plans[el.dataset.planPrice],
  ...
  async function loadBillingConfig() {
    try {
      ...
      billingConfig = {
        ...billingConfig,
        ...body,
        plans: { ...billingConfig.plans, ...(body.plans || {}) }
      };
      applyPlanUI(billingConfig);
      const disclaimer = $("#planDisclaimer");
      if (disclaimer && billingConfig.enabled)
        disclaimer.textContent =
          "Cobrança mensal recorrente. O pagamento é concluído no ambiente seguro do Mercado Pago e o acesso é processado após a confirmação.";
    } catch (error) {
  ...
  function openCheckout(plan) {
    ...
    $("#checkoutPlanSeats").textContent =
      String(data.seats) + " usuário" + (Number(data.seats) === 1 ? "" : "s");
    $("#checkoutPlanStorage").textContent = String(data.storageGb) + " GB";
    checkoutStatus.textContent = billingConfig.enabled
      ? ""
      : "A contratação online está sendo ativada. Se esta mensagem persistir, tente novamente em instantes.";
  ...
  checkoutForm?.addEventListener("submit", async (e) => {
    ...
    if (!billingConfig.enabled) {
      checkoutStatus.textContent =
        "O checkout ainda não está liberado para transações. Tente novamente após a ativação.";
  ...
  function resultMessage(data) {
    const status = data.status,
      provision = data.provisioningStatus;
    if (status === "approved") {
      if (provision === "activated") {
        return {
          cls: "approved",
          html: "<strong>Pagamento aprovado e acesso liberado.</strong> As instruções do primeiro acesso foram enviadas para o e-mail usado na contratação. Se esse e-mail já possuía uma conta, você pode entrar normalmente. <small>Não encontrou a mensagem? Na tela de acesso, use “Esqueci minha senha”.</small>"
        };
      }
      return {
        cls: "pending",
        html: "<strong>Pagamento aprovado.</strong> A criação segura do seu acesso está sendo concluída automaticamente. Você não precisa enviar comprovante nem solicitar liberação manual."
      };
    }
    if (status === "pending" || status === "created") {
      return {
        cls: "pending",
        html: "<strong>Pagamento pendente.</strong> Aguardando a confirmação do Mercado Pago. Você não precisa enviar comprovante."
      };
    }
    if (status === "rejected")
      return {
        cls: "problem",
        html: "<strong>Pagamento recusado.</strong> O acesso não foi liberado. Tente novamente pelo meio de pagamento disponível."
      };
    if (status === "in_mediation")
      return {
        cls: "pending",
        html: "<strong>Pagamento em análise.</strong> A liberação fica suspensa até a definição do provedor."
      };
        ...
        html:
          ...
          (status === "refunded"
            ...
            : status === "charged_back"
              ? "com chargeback"
              : "cancelado") +
          ".</strong> O acesso associado fica bloqueado."
      };
    return {
      cls: "pending",
      html: "<strong>Status do pagamento:</strong> " + String(status || "em processamento") + "."
    };
  }
  async function showPaymentReturn(orderId) {
    ...
    checkoutResult.innerHTML =
      "<strong>Verificando pagamento…</strong> Consultando a confirmação do Mercado Pago.";
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        const r = await fetch("/api/payment-status?order=" + encodeURIComponent(orderId), {
          cache: "no-store"
        });
        ...
        if (saasHref) {
          try {
            loginHref = new URL("/login?first=1", saasHref).href;
          } catch {}
        }
        checkoutResult.innerHTML =
          msg.html +
          (data.status === "approved" && data.provisioningStatus === "activated" && loginHref
            ? '<div class="result-actions"><a href="' +
              loginHref +
              '">Fazer primeiro acesso →</a></div>'
            : "");
  ...
  const checkoutReturn =
    params.get("checkout") === "return"
      ? params.get("order") || sessionStorage.getItem("ed:last-order")
      : "";
  if (checkoutReturn) {
