...
import {PLAN_CATALOG,BILLING_CONTRACT_VERSION,BILLING_CONTRACT_FINGERPRINT} from "../../../contracts/billing-v1.js";
import {
...
async function verifyRequest(req,body){
  ...
  if(!secret)throw Object.assign(new Error("Ponte de cobrança não configurada."),{status:503,code:"billing_bridge_not_configured"});
  if(!timestamp||!signature||!Number.isFinite(ts))throw Object.assign(new Error("Autenticação da gestão de assinatura ausente."),{status:401,code:"subscription_signature_missing"});
  ...
  if(!ok)throw Object.assign(new Error("Autenticação da gestão de assinatura inválida."),{status:401,code:"subscription_signature_invalid"});
}
...
async function billingContext({organizationId,email}){
  ...
  if(!account)throw Object.assign(new Error("Assinatura não vinculada a este escritório."),{status:404,code:"subscription_not_found"});
...
async function currentUsage(organizationId){
  const [seats,storage]=await Promise.all([
    db.query("SELECT count(*)::int AS used FROM office_memberships WHERE organization_id=$1::uuid AND active=true",[organizationId]),
    db.query("SELECT COALESCE(sum(size_bytes),0)::bigint AS used FROM office_files WHERE organization_id=$1::uuid",[organizationId])
  ]);
  return {
...
async function syncMain({order,account,action,plan,provider,requestId}){
  ...
  if(!response.ok)throw Object.assign(new Error("O SaaS respondeu "+response.status),{code:"saas_sync_failed"});
  return true;
}

async function recordManagementEvent({requestId,order,action,status,targetPlan,organizationId,syncPending}){
  await db.query(
...
export default async function(req,res){
  ...
  try{
    if(version!==SUBSCRIPTION_MANAGEMENT_VERSION){
      return fail(res,409,"Contrato de gestão de assinatura incompatível.","subscription_contract_mismatch",{expectedVersion:SUBSCRIPTION_MANAGEMENT_VERSION});
    }
    if(!requestId||!organizationId||!email||!SUBSCRIPTION_MANAGEMENT_ACTIONS.has(action)){
      return fail(res,400,"Solicitação de gestão de assinatura inválida.","subscription_request_invalid");
    }
    ...
    if(account.status!=="active"){
      return fail(res,409,"A assinatura não está ativa para alterações.","subscription_not_active");
    }
    if(!order?.provider_subscription_id){
      return fail(res,409,"A cobrança recorrente ainda não está vinculada a uma assinatura do Mercado Pago.","provider_subscription_missing");
    }
    ...
    if(action==="change_plan"){
      ...
      if(usage.seats>target.seats){
        return fail(res,409,"O plano escolhido possui menos acessos do que os atualmente utilizados.","seat_limit_conflict",{used:usage.seats,limit:target.seats});
      }
      if(usage.storageBytes>target.storageGb*GB){
        return fail(res,409,"O plano escolhido possui menos armazenamento do que o atualmente utilizado.","storage_limit_conflict",{usedBytes:usage.storageBytes,limitBytes:target.storageGb*GB});
      }
      if(String(account.subscription_plan||"")===targetPlan){
        const provider=await providerFor(order).catch(()=>null);
        return res.json({ok:true,idempotent:true,plan:targetPlan,provider:provider?safeProviderView(provider):null});
      }
      ...
      if(["canceled","cancelled"].includes(beforeStatus)){
        return fail(res,409,"A assinatura já está cancelada no provedor.","provider_subscription_cancelled");
      }

      const provider=await mpFetch("/preapproval/"+encodeURIComponent(order.provider_subscription_id),{
        method:"PUT",
      ...
      try{
        await syncMain({order:fresh,account,action:"activate",plan:targetPlan,provider,requestId});
        await db.query("UPDATE sales_orders SET provisioning_status='activated',automation_action='activate',failure_reason='',updated_at=now() WHERE id=$1",[order.id]);
      }catch(error){
        syncPending=true;
        await db.query("UPDATE sales_orders SET provisioning_status='retry',automation_action='activate',failure_reason=$2,updated_at=now() WHERE id=$1",[order.id,clean(error?.message||error,500)]);
      }

      await recordManagementEvent({requestId,order:fresh,action,status:provider.status,targetPlan,organizationId,syncPending});
    ...
    const before=await providerFor(order);
    const alreadyCancelled=["canceled","cancelled"].includes(clean(before?.status,40).toLowerCase());
    const provider=alreadyCancelled?before:await mpFetch("/preapproval/"+encodeURIComponent(order.provider_subscription_id),{
      method:"PUT",
      idempotencyKey:requestId,
    ...
    let syncPending=false;
    try{
      await syncMain({order:fresh,account,action:"block",plan:account.subscription_plan,provider,requestId});
      await db.query("UPDATE sales_orders SET provisioning_status='blocked',automation_action='block',failure_reason='',updated_at=now() WHERE id=$1",[order.id]);
    }catch(error){
      syncPending=true;
      await db.query("UPDATE sales_orders SET provisioning_status='retry',automation_action='block',failure_reason=$2,updated_at=now() WHERE id=$1",[order.id,clean(error?.message||error,500)]);
    }

    await recordManagementEvent({requestId,order:fresh,action,status:internalStatus,targetPlan:"",organizationId,syncPending});
    return res.status(syncPending?202:200).json({
      ok:true,
      cancelled:true,
      syncPending,
    ...
    return fail(
      ...
      Number(error?.status)>=400&&Number(error?.status)<500?clean(error?.message,240):"Não foi possível concluir a gestão da assinatura.",
