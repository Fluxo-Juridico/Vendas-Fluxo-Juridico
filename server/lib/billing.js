...
export function validateProviderPayment(order, payment) {
  ...
  const paymentSubscriptionId = clean(payment?.preapproval_id || payment?.subscription_id, 120);
  ...
  if (
    ...
    paymentSubscriptionId !== orderSubscriptionId
  ) {
...
export async function settings() {
  ...
  const billingReady =
    Boolean(values[2]) && Boolean(values[3]) && Boolean(values[4]) && Boolean(values[5]);
  const fromCatalog = (key) =>
    Object.fromEntries(Object.entries(PLAN_DEFAULTS).map(([name, plan]) => [name, plan[key]]));
...
export async function mpFetch(path, { method = "GET", body, idempotencyKey = "" } = {}) {
  const token = String((await config.get("MERCADO_PAGO_ACCESS_TOKEN")) || "").trim();
  if (!token) {
  ...
  if (!response.ok) {
    ...
    const cause = Array.isArray(data?.cause)
      ...
      : "";
    const diagnostic =
      "Mercado Pago " + response.status + ": " + providerMessage + (cause ? " | " + cause : "");
  ...
  try {
    const payload = {
      eventId: [
        String(order.id),
        status,
        String(order.provider_payment_id || order.provider_subscription_id || ""),
        plan,
        String(entitlement.seatLimit),
        String(entitlement.storageLimitGb),
        String(entitlement.contractVersion)
      ].join(":"),
      orderId: order.id,
      action,
    ...
    const response = await signedBridgeRequest(cfg.saasBaseUrl + "/api/billing-provision", payload);

    if (!response.ok) throw new Error("SaaS respondeu " + response.status);

    const provisioningStatus =
      action === "activate" ? "activated" : action === "block" ? "blocked" : "review";
...
export async function upsertLeadForCheckout({ name, email, phone, firmName, plan }) {
  ...
  if (found.rows[0]) {
    await db.query(
      ...
      [found.rows[0].id, name, phone, firmName, "Checkout iniciado — plano " + plan]
    );
