 ...
 *
 * Keep plan defaults here. Runtime Hatchable config may override prices,
 * seat limits and storage limits without requiring a deploy.
 */
...
export async function settings(){
  ...
  const billingReady=Boolean(values[15])&&Boolean(values[16])&&Boolean(values[17]);
  ...
  return {
    ...
    saasBaseUrl:String(values[14]||"https://brito-vilarinho.hatchable.site").replace(/\/$/,""),
...
export async function tryProvision(order){
  ...
  try{
    const payload={
      ...
      plan,
      seats:seatsFor(plan,cfg),
      storageGb:storageGbFor(plan,cfg),
