...
if(cronSecret&&cronSecret.length<24)errors.push("CRON_SECRET must contain at least 24 characters.");
if(saasBaseUrl)httpsUrl("SAAS_BASE_URL",saasBaseUrl);

for(const plan of ["SOLO","ESSENCIAL","PROFISSIONAL","PREMIUM"]){
  positive(`PLAN_${plan}_PRICE`,process.env[`PLAN_${plan}_PRICE`]||({SOLO:99,ESSENCIAL:197,PROFISSIONAL:297,PREMIUM:497}[plan]));
  positive(`PLAN_${plan}_SEATS`,process.env[`PLAN_${plan}_SEATS`]||({SOLO:2,ESSENCIAL:3,PROFISSIONAL:10,PREMIUM:20}[plan]));
  positive(`PLAN_${plan}_STORAGE_GB`,process.env[`PLAN_${plan}_STORAGE_GB`]||({SOLO:5,ESSENCIAL:15,PROFISSIONAL:25,PREMIUM:100}[plan]));
}
...
console.log(JSON.stringify({ok:true,app:"fluxo-juridico-vendas",billingContract:"billing-v1"},null,2));
